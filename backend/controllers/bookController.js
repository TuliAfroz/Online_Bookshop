...
export const createBook = async (req, res) => {
  const {
    ...
    Author_Name, Publisher_ID, Price, Cover_Image_URL
  } = req.body;

  // Remove Category_ID from required fields
  if (!Book_ID || !Title || !Author_Name || !Publisher_ID || !Price) {
  ...
  try {
    ...
    const authorCheck = await pool.query(
      'SELECT * FROM Author WHERE LOWER(author_name) = LOWER($1)',
      [Author_Name]
    );
    if (authorCheck.rows.length === 0) {
      return res.status(200).json({
  redirect: '/publisher/dashboard?tab=add-author',
  message: `Author "${Author_Name}" not found. Please add the author.`
});
    }
    const Author_ID = authorCheck.rows[0].author_id;
